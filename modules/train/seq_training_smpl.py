...
def _loop(
    ...
    for i, batch in tqdm(enumerate(loader), total = len(loader), desc= f'Epoch {epoch}: {name}-loop'):
        ...
        poses_pred = poses_pred.reshape(-1, 72)
        verts_pred = smpl(beta= betas_pred, pose = poses_pred)

        # Get 3d Joints from smpl-model (dim: 17x3) and normalize with Pelvis
        joints3d_smpl_gt = smpl.get_h36m_joints(verts_gt)
        joints3d_pred = smpl.get_h36m_joints(verts_pred)
        ...
        #Normalize Groundtruth & Prediction
        verts_gt = verts_gt - pelvis_gt[:, None, :]
        verts_pred = verts_pred - pelvis_pred[:, None, :]
        ...
        if scale:
            ...
            verts_gt = verts_gt/scale_smpl_gt
        ...
        preds = {"SMPL": (betas_pred, poses_pred), "VERTS": verts_pred, "KP_3D": joints3d_pred}
        targets = {"SMPL": (betas_gt, poses_gt), "VERTS": verts_gt, "KP_3D": joints3d_smpl_gt}
